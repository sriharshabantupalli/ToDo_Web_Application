...
def home(request):  # This will render the main home page
    stored_messages = messages.get_messages(request)
    success_message = ''
    for msg in stored_messages:
        success_message = msg
        ...
        if request.method == "POST":
            ...
            if request.GET.get('Id') is not None:
                if request.GET.get('status') == '1':
                    Task.objects.filter(taskId=request.GET.get('Id')).update(
                        taskStatus=1)  # Here I am updating the task status(completed, not started, in progress)
...
def UpdateTask(request):  # This is the function to handle the update task
    if request.user.is_authenticated:
        ...
        return render(request, 'UpdateTask.html',
                      {'updateTaskTitle': update_task_title, 'updateTaskDescription': update_task_description,
                       'updateTaskId': TaskId})
    else:
        return HttpResponse('404 - Not Found')
...
def handleSignup(request):  # This function will handle the Signup
    if request.method == 'POST':
...
def handleLogin(request):  # This function will handle the Login
    if request.method == 'POST':
        # Get the post parameters
        username = request.POST['username']
...
def textutilAnalyze(
        request):  # This function will handle the text analysis and render TextAnalyze page with optimised text.
    if request.user.is_authenticated:
        ...
        if extraspaceremover == 'on':
            ...
            if len(purpose) > 0:
                ...
                purpose = purpose + "Removed ExtraSpace"
        if removepunc != "on" and fullcaps != 'on' and newlineremover != 'on' and extraspaceremover != 'on' and len(
                djtext) > 0:
